// File: ui/theme/Color.kt
package com.kotlin.tictactoe.theme

import androidx.compose.ui.graphics.Color

val md_theme_light_primary = Color(0xFF006C4F)
val md_theme_light_onPrimary = Color(0xFFFFFFFF)
val md_theme_light_secondary = Color(0xFF4A6354)
val md_theme_light_onSecondary = Color(0xFFFFFFFF)
val md_theme_light_background = Color(0xFFFDFDF9)
val md_theme_light_onBackground = Color(0xFF1B1C18)
val md_theme_light_surface = Color(0xFFFDFDF9)
val md_theme_light_onSurface = Color(0xFF1B1C18)
val md_theme_light_error = Color(0xFFBA1A1A)
val md_theme_light_onError = Color(0xFFFFFFFF)

val md_theme_dark_primary = Color(0xFF56D7A4)
val md_theme_dark_onPrimary = Color(0xFF003828)
val md_theme_dark_secondary = Color(0xFFB4CCB8)
val md_theme_dark_onSecondary = Color(0xFF1F352B)
val md_theme_dark_background = Color(0xFF1B1C18)
val md_theme_dark_onBackground = Color(0xFFE2E3DC)
val md_theme_dark_surface = Color(0xFF1B1C18)
val md_theme_dark_onSurface = Color(0xFFE2E3DC)
val md_theme_dark_error = Color(0xFFFFB4AB)
val md_theme_dark_onError = Color(0xFF690005)
